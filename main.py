...
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            name = f'Join @Rexodas2023 {name1[:90]}'
            ...
            try:                               
                cc = f'`{str(count).zfill(3)}.{name1}.mkv`\n\n**Batch Name :** `{b_name}`\n\nJoin @Rexodas2023 @RexodasOfficial For More Exclusive Batches 🔥🖤\n\n**Downloaded By :** {CR}'
                cc1 = f'`{str(count).zfill(3)}.{name1}`\n\n**Batch Name :** `{b_name}`\n\nJoin @Rexodas2023 @RexodasOfficial For More Exclusive Batches 🔥🖤\n\n**Downloaded By :** {CR}'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1, thumbnail=thumb)
                    ...
                    try:
                        ...
                        copy = await bot.send_document(chat_id=m.chat.id,document=f'{name}.pdf', caption=cc1, thumbnail=thumb)
                        await copy.copy(chat_id = -1001296608859)
                        count += 1
