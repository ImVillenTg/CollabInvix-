...
async def send_vid(bot: Client, m: Message, cc, filename, thumb, name):
    ...
    #    shell=True)
    subprocess.run(f'ffmpeg -i "{filename}" -ss 00:00:12 -filter_complex "drawtext=text=\'@RolexEmpire\':fontcolor=black@0.5:fontsize=(w/20):x=10:y=h-th-10+2:shadowcolor=white:shadowx=2:shadowy=2" -vframes 1 "{filename}.jpg"', shell=True)
